"""
This is a specific graph net training file for optimizing 2-label training
...
class Trainer:
    ...
    def step(self, train_graphs: list, train_labels: list, test_graphs: list, test_labels: list, epoch: int):
        ...
        self.stats = self.stat.append(epoch_stats, ignore_index = True)
        return train_loss
...
model = graphs.GIModel(nlabels = len(labels), hidden_layers = 2, hidden_size = 256)
t = Trainer(model)
...
for epoch in tqdm.trange(epochs):
    ...
    analysis.bare_roc(np.array(predsm), true_labels, ref_label, f'roc_{output}', epoch_roc=True, epoch_num = epoch)
